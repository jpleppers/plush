...
(function() {
  this.Plush = (function() {
    function Plush(element, options) {
      ...
      this.options.dragDrop = (this.options.drag_drop != null) || (this.element.data('drag-drop') != null);
      ...
      this.inputContainer.on('click', '.plush-placeholder, .plush-caret', function(event) {
        ...
        if (_this.options.multiple && $(event.target).hasClass('plush-input-wrapper')) {
          return _this.show();
        }
    ...
    Plush.prototype.handleListKeyPress = function(event) {
      ...
      if (event.which === 13) {
        ...
        if ((this.options.multiple != null) && this.options.multiple) {
    ...
    Plush.prototype.moveMultiSelectItem = function(item) {
      var $option, $prevItem, $prevOption;
      $option = $("[value=" + (item.data('value')) + "]", this.element);
