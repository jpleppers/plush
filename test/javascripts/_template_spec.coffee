...
describe "List item template", ->
  before ->
    ...
    @listItem = $.handlebar('plush_list_item', {value: @item.value(), label: @item.label})
...
describe "Image list item template", ->
  before ->
    @item = window.testItems.randomItem()
    @listItem = $.handlebar('plush_image_list_item', {value: @item.value(), label: @item.label, image: @item.image(), description: @item.description()})
